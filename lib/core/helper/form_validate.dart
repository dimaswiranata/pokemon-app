...
class FormValidate {
  ...
  static String? validatePhone(String? value) {
    const pattern = r'^\+[0-9]{9,15}$'; // Update the pattern to allow an optional plus sign
    final regex = RegExp(pattern);
    if (value!.isEmpty) {
      return 'Enter a phone number';
    } else if (value.length < 10 || value.length > 15) {
      return 'Please enter 10 - 15 digits only';
  ...
  static String? validateInternationalPhone(String? value) {
    ...
    if (value!.isEmpty) {
      return 'Enter a phone number';
    } else if ((value.length < 9 || value.length > 15) || !regex.hasMatch(value)) {
  ...
  static String? validateUsername(String? value) {
    ...
    if (value!.isEmpty) {
      ...
      return 'Username can contain numbers (0-9), underscore (_), and perionds(.)';
    } else {
      return null;
    }
  }

  static String removeZeroFirst(String value) {
    if (value[0] == "0") {
...
class Strings {
  static const String fieldReq = 'required';
